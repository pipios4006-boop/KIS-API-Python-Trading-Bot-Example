...
# 🚨 NEW: [타임쉴드 텍스트 오버라이드] 04:07 타임쉴드 가동 상태 및 04:30 구간 1분 유지 감시 로직을 관제탑 UI에 100% 팩트 표출 완료.
# ==========================================================
import logging
import datetime
...
class AvwapConsolePlugin:
        ...
        if is_assassin_active:
            ...
            if is_avwap_hybrid:
                if is_mission_complete:
                    ...
                    # 🚨 NEW: 04:07 타임쉴드 및 04:30 1분 유지 조건 분기 렌더링 락온
                    if now_est.time() < datetime.time(4, 7):
                        msg += f"▫️ 교전 상태: <b>ON (04:07 EST 이전 타임쉴드 가동 중)</b>\n"
                    elif now_est.time() < datetime.time(4, 30):
                        msg += f"▫️ 교전 상태: <b>ON (04:07~04:30 EST VWAP 1분 유지 감시 중)</b>\n"
                    else:
    ...
    def get_avwap_warning_menu(self, ticker):
        ...
        # 🚨 NEW: 04:07 타임쉴드 안내 텍스트 롤오버 락온
        msg += f"▫️ <b>타점:</b> <b>04:07 EST 타임쉴드 해제 후 ~ 04:30 EST</b> 구간은 VWAP 상회 <b>1분 유지 시</b> 진입, <b>04:30 이후</b> 즉각 상향 돌파 요격 (매도 1호가 지정가)\n"
